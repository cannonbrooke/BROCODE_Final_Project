...
subtitle: BUS696 Final Project v11
output:
  html_document:
    df_print: paged
...
dataclean <- data %>%
  ...
  mutate(ID = as.numeric(ID)) %>% #adding number of degrees
...
summarize(group_by(dataclean, Sex), 
          avg_giving = mean(Lifetime.Giving, na.rm = TRUE),
...
lasso_fit <- cv.glmnet(Lifetime.Giving ~ school1_fct + maj1_fct + degree1_fct + donorseg_fct,
                       data = data_train,
                       #Alpha 1 for lasso
                       alpha = 1)
...
corrplot_data <- dataclean[-c(1:2,5:33,35:47,49,51:56,58:64,66:67,69,71:122,126:131)]
